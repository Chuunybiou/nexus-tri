...
describe("Inventory navigation", () => {
  it("renders Inventory in desktop and mobile navigation", async () => {
    ...
    const presence = (v: unknown) =>
      SERVICES_DE_COMPTE ? expect(v).toBeTruthy() : expect(v).toBeNull();
    presence(
      desktop.querySelector(
        '[data-page="page-inventory"][data-i18n="main.inventory"]',
      ),
    );
    presence(mobile.querySelector('[data-page="page-inventory"]'));
    presence(
  ...
  siComptes(
    "routes an actual navigation click with the active tab and restores deep links",
    async () => {

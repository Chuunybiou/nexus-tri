...
export const PlayerSchema = z.object({
  ...
  teamIndex: zb.uint().optional(),
  // Same class of field as teamIndex: a simulation input that every client
  // must agree on, so the server stamps it rather than each client trusting
  // its own copy. Optional so records written before factions existed still
  // parse; readers fall back to DEFAULT_FACTION.
...
export const ClientJoinMessageSchema = z.object({
  ...
  gitCommit: z.string().max(64).optional(),
  // The faction picked in the lobby. A request, not a fact — the server
  // stamps the authoritative value into GameStartInfo.players.
  faction: FactionSchema.optional(),
});

...
export class Config {
  ...
  private costWrapper(
    ...
    return (game: Game, player: Player, extraUnits: number = 0) => {
      ...
      return scaleGoldPerMille(
        BigInt(costFn(numUnits + extraUnits)),
        factionTraits(player.faction()).unitCostPerMille,
      );
    };
  }
  ...
  attackAmount(attacker: Player, defender: Player | TerraNullius) {
    const base =
      attacker.type() === PlayerType.Bot
        ? attacker.troops() / 20
        : attacker.troops() / 5;
    // Applied last, on top of the bot/human split: a Swarm bot and a Swarm
    // human both commit their faction's share of whatever they would have.
    return applyPerMille(
      base,
      factionTraits(attacker.faction()).attackPerMille,
    );
  }
  ...
  troopIncreaseRate(player: Player | PlayerView): number {
    ...
    // Before the clamp, so a growth buff speeds up the approach to maxTroops
    // without letting anyone exceed it.
    toAdd = applyPerMille(
      toAdd,
      factionTraits(player.faction()).troopGrowthPerMille,
    );
  ...
  goldAdditionRate(player: Player | PlayerView): Gold {
    ...
    return scaleGoldPerMille(
      BigInt(Math.floor(Number(baseRate) * multiplier)),
      factionTraits(player.faction()).goldRatePerMille,
    );
  }

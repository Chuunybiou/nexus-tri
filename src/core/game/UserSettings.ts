...
import { DEFAULT_FACTION, Faction, isFaction } from "./Factions";
...
const FACTION_KEY = "settings.faction";
...
export class UserSettings {
  ...
  /**
   * The faction the player last picked in the lobby.
   *
   * Validated on read rather than trusted: this comes back from localStorage,
   * which the player can edit and which still holds values written by older
   * builds. An unrecognised value falls back to the balance anchor instead of
   * reaching the join message.
   */
  selectedFaction(): Faction {
    const stored = this.getString(FACTION_KEY, DEFAULT_FACTION);
    return isFaction(stored) ? stored : DEFAULT_FACTION;
  }

  setSelectedFaction(faction: Faction) {
    this.setString(FACTION_KEY, faction);
  }

...
/**
 * Tutorial card: outlined, not filled.
 *
 * Two solid blocks side by side gave the page no primary action, and the gold
 * it used is now the Ascendant faction colour — a saturated fill here would
 * read as a faction cue rather than a button. Solo is the one solid; everything
 * else states itself with an edge.
 */
const TUTORIAL_ACTION =
  "bg-transparent border border-cyber-yellow/60 text-cyber-yellow hover:bg-cyber-yellow/10 hover:border-cyber-yellow active:bg-cyber-yellow/20 hover:scale-y-105 hover:scale-x-[1.01]";

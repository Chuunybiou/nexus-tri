...
export class SinglePlayerModal extends BaseModal {
  ...
  /**
   * Opens the modal already set to `map`.
   *
   * A real method rather than letting callers poke `selectedMap`: that field is
   * @state and private, and setting it alone would skip clearing the
   * random-map flag and reloading the nation count, so the modal would open
   * showing one map and start another.
   *
   * It stops at the modal instead of starting the game, so the player still
   * chooses bots, difficulty and the rest — clicking a name in a catalogue is
   * browsing, not a commitment to launch.
   */
  public openWithMap(map: GameMapType) {
    this.handleMapSelection(map);
    this.open();
  }
              ...
              players: [
                {
                  ...
                  cosmetics,
                  // Singleplayer has no server to stamp this, so the lobby
                  // pick goes straight into the start info the engine reads.
                  faction: new UserSettings().selectedFaction(),
                },
              ],
              config: {

...
class ThemeProvider {
  ...
  current(): Theme {
    ...
    // Defaults ON. The flat Bot team colour is a SINGLE value (#d1cdc7 in the
    // default theme), so every tribe rendered identically and two adjacent
    // tribes had no visible boundary at all — same fill, same border. The
    // classic pool gives each one its own colour, which is what makes a border
    // between two bots exist in the first place. Players who prefer the flat
    // look can still turn it off in the graphics settings.
    theme.useClassicBotColors = overrides.classicBotColors ?? true;
    return theme;
  }

...
export function createRenderer(
  ...
  // Guarded rather than following the assign-anyway style above: this banner is
  // a purely informational overlay, and a shell that ships without the element
  // should lose the warnings, not the whole renderer.
  const factionStatus = document.querySelector(
    "faction-status",
  ) as FactionStatus | null;
  if (factionStatus instanceof FactionStatus) {
    factionStatus.game = game;
  } else {

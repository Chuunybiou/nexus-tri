...
## This is a fork — read `docs/FORK.md` first

This repo is an original military-industrial / sci-fi reskin of OpenFront built around three
asymmetric factions (`Vanguard` / `Swarm` / `Ascendant`). **`docs/FORK.md` is the handoff**:
what has been built, what is deliberately not built yet, and the traps that have already
cost time — including why you must run the tests on Node 22 and not Node 26, and which three
test files fail on a clean checkout for reasons unrelated to any change you make.
...
| `docs/FORK.md`              | Fork state, decisions and known traps  |
| `src/core/game/Factions.ts` | Faction traits and per-mille modifiers |
